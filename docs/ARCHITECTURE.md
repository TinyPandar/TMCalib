...
- 输入网格：32×24、128×96、160×120 或 128×128；
- DMD 映射：全 1024×768、128×96 对齐重复或 160×120 最近邻扩展到 256×192 超像素后铺满，或中央 512×512；
...
`fourfold_128x96` 以 32×24 为基准扩大 4 倍，得到 128×96 的逻辑输入。每个输入对应严格对齐的 8×8 DMD 宏像素；编码时先沿两个方向各重复 2 次，得到完整 256×192 光学超像素网格，再由 4×4 `holo_SP` tile 编码。因此所有 1024×768 微镜均属于有效区，没有外围 zero padding。数据 metadata 以 `aligned_repeat2_128x96_to_256x192_v1` 固定这一映射契约。
...
- `fourfold_128x96` 通过薄配置层复用 128×128 相机输出的硬件采集、整数倍聚焦编码与重建流程。
...
3. 把 Pattern 编码定义为 32×24、128×96 全场、160×120 全场和 128×128 中央有效区四个策略。
