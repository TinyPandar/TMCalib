...
采集采用单图独立同步，而不是把45张图以500 Hz连续播放：每次只加载并循环投影一张DMD图，等待50 ms后才开启一个干净的相机会话拍摄一帧，然后停止DMD触发、关闭相机并清空序列。即使操作系统短暂停顿或相机漏过硬件触发，下一次触发仍对应同一张DMD图，因此不会造成图样与相机帧错位。

每轮结束后程序立即打印黑场和满幅参考的平均强度。若满幅参考不高于黑场，采集会立刻停止，避免继续产生不可用数据。
...
- 日志若出现 `estimated_missed_triggers > 0`，连续批量采集结果不能用于本标定；单图独立同步模式不依赖连续帧编号。
