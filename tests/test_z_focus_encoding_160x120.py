...
class FocusEncoding160x120Tests(unittest.TestCase):
    # Import the profile lazily because it intentionally specializes globals
    # in the shared 128-output acquisition module at import time.
    @staticmethod
    ...
    def test_profile_enables_its_specialized_gpu_encoder(self):
        ...
        self.assertTrue(controller.focus_encoding_use_gpu)

    def test_cuda_batch_is_bit_exact_with_numpy_reference(self):
        module = importlib.import_module("calibrate_160x120")
        ...
        rows = (
            rng.standard_normal(
                (3, module.INPUT_HEIGHT * module.INPUT_WIDTH)
            )
            + 1j
            * rng.standard_normal(
                (3, module.INPUT_HEIGHT * module.INPUT_WIDTH)
        ...
        numpy_patterns, numpy_errors = (
            controller._build_focus_hologram_batch(
                rows,
                lut_cache=lut_cache,
            )
        )
        ...
        self.assertEqual(numpy_errors, cuda_errors)
        np.testing.assert_array_equal(cuda_patterns, numpy_patterns)
        self.assertEqual(
            controller.last_focus_encoding_backend,
            "GPU cuda:0 (160x120 nearest-fill)",
        )
        self.assertIsNone(controller.last_focus_encoding_fallback_error)

    def test_cuda_batch_rejects_invalid_tm_rows(self):
        module = importlib.import_module("calibrate_160x120")
        if not module.core.torch.cuda.is_available():
            self.skipTest("CUDA is unavailable")
