# Transmission-matrix calibration: 128 x 128 input and selectable 26 x 26 polarization output.
...
PATTERN_128_CONFIG = {
    ...
    "sets": {
        "4N": {
            ...
            "measurement_filename": "measurements_128_px4_active512_full_I0_memmap.npy",
            "tm_memmap_filename": "transmission_matrix_128_px4_active512_I0_memmap.npy",
            "reconstructed_filename": "reconstructed_field_128_px4_active512_I0.npy",
            "error_curve_filename": "ggs21_error_curve_128_px4_active512_I0.npy",
            ...
            "reconstruction_metadata_filename": "tm_reconstruction_128_px4_active512_I0.json",
        },
        "8N": {
            ...
            "measurement_filename": "measurements_128_px4_active512_8N_full_I0_memmap.npy",
            "tm_memmap_filename": "transmission_matrix_128_px4_active512_8N_I0_memmap.npy",
            "reconstructed_filename": "reconstructed_field_128_px4_active512_8N_I0.npy",
            "error_curve_filename": "ggs21_error_curve_128_px4_active512_8N_I0.npy",
            ...
            "reconstruction_metadata_filename": "tm_reconstruction_128_px4_active512_8N_I0.json",
        },
    },
}


def get_active_128_pattern_config(polarization_channel="I0"):
    ...
    for key in (
        ...
        config[key] = channelized_filename(config[key], channel)
    config["polarization_channel"] = channel
    config["name"] = active
...
class CameraHandler:
    def __init__(self, cam_index, save_path, polarization_channel="I0"):
    ...
    def configure_roi(self):
        """Configure a centered 52x52 raw ROI for a 26x26 polar image."""
        if not self.cam:
            return False
    ...
    def run(self):
        ...
        try:
            ...
            # Extract the 0-degree micro-polarizer channel.  The
            # 52x52 raw Polarized8 ROI becomes one 26x26 Mono8 image.
...
class DMDController:
    def __init__(self, camera_handler=None):
        ...
        self.polarization_channel = normalize_polarization_channel(
            getattr(self.camera, "polarization_channel", "I0")
        )
        self.full_pattern_config = get_active_128_pattern_config(
            self.polarization_channel
        )
        ...
        self.measurement_filename = (
            "measurements_128_px4_active512_test_{}_memmap.npy".format(
                self.polarization_channel
            )
            if self.test_mode
    ...
    def _validate_tm_shape(self, H):
        roi_w = getattr(self.camera, "roi_width", 26)
        roi_h = getattr(self.camera, "roi_height", 26)
    ...
    def project_and_caption(self, pattern=None):
        ...
        image_data = np.empty((batch, self.camera.roi_height, self.camera.roi_width), dtype=np.uint16)
    ...
    def run_reconstruction(self):
        ...
        try:
            ...
            config = ReconstructionConfig(
                ...
                output_shape=(
                    getattr(self.camera, "roi_height", 26),
                    getattr(self.camera, "roi_width", 26),
                ),
    ...
    def conjugate_focus_at_position(self, target_x, target_y, px=4, ds_method='mean'):
        ...
        try:
            ...
            # H的形状应该是 (N_out, N_in)
            # N_out = 676 (26x26 camera ROI pixels)
    ...
    def conjugate_focus_with_partial_tm(
        ...
        if tm_filename is None:
            tm_filename = (
                "reconstructed_field_128_px4_active512_{}_test_subset.npy".format(
                    self.polarization_channel
                )
            )
        if metadata_filename is None:
            metadata_filename = (
                "tm_reconstruction_128_px4_active512_{}_test_subset.json".format(
                    self.polarization_channel
                )
            )
        result = {
    ...
    def pixelwise_focus_average_pbr(
        ...
        if output_dir is None:
            output_dir = os.path.join(
                os.path.dirname(os.path.abspath(__file__)),
                "pixelwise_focus_results_128_px4_active512_{}".format(
                    self.polarization_channel
                ),
            )
...
class Application(tk.Tk):
    def __init__(self, polarization_channel="I0"):
        ...
        self.title(
            "DMD TM Calibration - 128x128 Input / px=4 / camera 26x26 / {}".format(
                self.polarization_channel
            )
        )
        ...
        self.camera = CameraHandler(
            cam_index=0,
            save_path="./camera_1",
            polarization_channel=self.polarization_channel,
        )
        ...
        self.title(
            "DMD TM Calibration - 128x128 / px=4 / camera 26x26 / {} - {}".format(
                self.polarization_channel, active_mode
            )
        )
    ...
    def setup_ui(self):
        ...
        ttk.Label(
            dmd_frame,
            text="128×128 input | 4×4 aligned | active 512×512 | camera 26×26",
        ...
        self.btn_pixelwise_focus = ttk.Button(
            focus_frame,
            text="Full 26x26 Camera Pixel-wise + PBR Report",
        ...
        self.pw_stride = tk.Spinbox(pw_frame, from_=1, to=26, width=5)
    ...
    def refresh_measurement_mode_ui(self):
        ...
        self.title(
            "DMD TM Calibration - 128x128 / px=4 / camera 26x26 / {} - {}".format(
                self.polarization_channel, mode_title
            )
        )
    ...
    def run_one_click_calibration(self):
        ...
        try:
            stride = max(1, min(26, int(self.pw_stride.get())))
        except Exception:
            stride = 1
        try:
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        choices=("I0", "I90"),
        default="I0",
        help="Polarization analyzer channel to extract from Polarized8 frames.",
    )
...
if __name__ == "__main__":
    args = _parse_args()
    app = Application(polarization_channel=args.channel)
