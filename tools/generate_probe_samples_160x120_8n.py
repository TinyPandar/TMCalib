"""Generate the 5x 160 x 120 -> 128 x 128 calibration dataset at 8N.

The 160 x 120 random phase field is expanded to the complete 256 x 192
...
dataset occupies about 134.5 GiB.
"""
...
DEFAULT_OUTPUT_DIR = "pregenerated_patterns_160x120_fill_8N_full"
...
def expand_phase_indices(phase_indices):
    """Expand source phase indices to the complete 192 x 256 SP grid."""
    ...
    if phase_indices.ndim != 3 or phase_indices.shape[1:] != expected_shape:
        raise ValueError(
            f"Phase-index batch shape {phase_indices.shape} must be (batch, {expected_shape[0]}, {expected_shape[1]})"
        )
...
def open_dataset(output_dir, resume, seed):
    ...
    if resume and all(
        ...
        if int(progress.get("random_seed", seed)) != int(seed):
            raise ValueError(
                f"Resume seed {seed} does not match saved seed {progress.get('random_seed')}"
            )
...
def generate_dataset(
    ...
    metadata = {
        "dataset_purpose": "full_160x120_to_128x128_calibration_8N",
        "probe_count": PROBE_COUNT,
        "probe_multiplier": PROBE_MULTIPLIER,
        "random_seed": int(seed),
        "input_shape": [INPUT_HEIGHT, INPUT_WIDTH],
        "expanded_input_shape": [EXPANDED_HEIGHT, EXPANDED_WIDTH],
        "expansion_method": "center_aligned_nearest_neighbor",
        ...
        "note": (
            "The 160x120 source is expanded to the full 256x192 optical-"
            "superpixel grid. Each expanded sample is encoded by one 4x4 "
            "holo_SP tile, filling the complete 1024x768 DMD."
        ),
    }
